...
producing a KEM ciphertext and a fresh 32-byte shared secret. The shared secret is used directly 
as the key for AES-256-GCM, so no separate symmetric key needs to be generated or transported. Separately, 
the sender signs a recipient-bound hash envelope that commits to the recipient's fingerprint, a digest of
 the plaintext, and a random salt. The plaintext and the signature are packed together into a small JSON inner payload, which is then encrypted under the KEM-derived AES-256-GCM key using a fresh 12-byte random nonce. The transmitted ciphertext is the concatenation of the KEM ciphertext, the AES-GCM nonce, and the authenticated symmetric ciphertext.
