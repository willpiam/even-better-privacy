...
\subsection{Supported Parameter Sets}
...
\begin{itemize}
  \item ML-DSA (Dilithium): ML-DSA-87
  \item SLH-DSA (SPHINCS+): SLH-DSA-SHA2-256s
  \item ML-KEM (Kyber): ML-KEM-1024
\end{itemize}
...
\begin{itemize}
  \item \textbf{Left leaf:} SHA-256(raw signing public key bytes)
  \item \textbf{Right leaf:} SHA-256(raw KEM public key bytes)
\end{itemize}
...
The bech32 prefix indicates what kind of keys are used. The prefix always begins with "ebp" and ends with the first occurence of "1". The remaining characters are always of even length and represent the key type. The first half of the characters represent the signing key type and the second half represent the KEM key type. The supported patterns include:
...
\begin{tabular}{|l|l|l|l|}
  ...
  \hline
  ML-DSA (Dilithium) & ML-KEM (Kyber) & \texttt{d} + \texttt{k} & \texttt{ebpdk1} \\
  SLH-DSA (SPHINCS+) & ML-KEM (Kyber) & \texttt{s} + \texttt{k} & \texttt{ebpsk1} \\
  \hline
\end{tabular}
\end{center}
