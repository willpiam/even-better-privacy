...
setMailLoaders({ loadMailAccount, loadStoredMailCredentials, renderStoredMailCredentials });
...
document.getElementById("verify-file-form").addEventListener("submit", async (e) => {
  ...
  await withLoading(btn, async () => {
    try {
      ...
      const payload = JSON.parse(payloadRaw);
      if (!payload || typeof payload !== "object") throw new Error("Signature payload must be a JSON object");
      if (payload.type !== "ebp-signed-file") throw new Error('Signature payload type must be "ebp-signed-file"');

      const expectedHash = typeof payload.fileHash === "string" ? payload.fileHash : "";
      if (!expectedHash) throw new Error("Signature payload missing fileHash");
      const signature = typeof payload.signature === "string" ? payload.signature : "";
      if (!signature) throw new Error("Signature payload missing signature");
      const identity = payload.identity;
      if (!identity || typeof identity !== "object") throw new Error("Signature payload missing identity public keys");
      ...
      const computedFingerprint = typeof computedFingerprintRes?.fingerprint === "string" ? computedFingerprintRes.fingerprint : "";
      ...
      if (!expectedFingerprint) {
        setResultBadge("verify-file-result", "invalid", "✗ Invalid");
        if (detailsOutput) detailsOutput.value = "Fingerprint missing from signature JSON (expected on payload or identity object).";
      ...
      if (signedMessageOutput) signedMessageOutput.value = reconstructedMessage;
  ...
  decFileDownloadBtn.addEventListener("click", async (e) => {
    ...
    try {
      ...
      const res = await api("/save-file", {
        method: "POST",
        body: JSON.stringify({ base64Content: state.decryptedFileResult.fileDataBase64, filename }),
      });
