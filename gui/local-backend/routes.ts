...
async function handleRequestInternal(req: Request): Promise<Response> {
	...
	try {
		...
		if (req.method === "POST" && url.pathname === "/api/v1/mail/oauth/start") {
			...
			mailOauthStarts.set(oauthState, { provider, createdAt: Date.now(), serverUrl });
		...
		if (req.method === "GET" && url.pathname === "/api/v1/mail/oauth/callback") {
			...
			try {
				const exchanged = await exchangeOAuthCode(pending.serverUrl, pending.provider, code);
		...
		if (req.method === "POST" && url.pathname === "/api/v1/decrypt") {
			...
			if (type === "ebp-encrypted-signed-message") {
				...
				const tryFetchFromServer = async (fp: string): Promise<ExternalIdentity | null> => {
					if (!ctx.server) return null;
					try {
						const res = await fetch(apiUrl(ctx.server, `/api/v1/identity/${fp}`));
						...
						return {
							...
							signingKeyDetails: (data.signingKeyDetails as ExternalIdentity["signingKeyDetails"]) ?? { variant: "ml_dsa87" },
							encryptionKeyDetails: (data.encryptionKeyDetails as ExternalIdentity["encryptionKeyDetails"]) ?? { variant: "ml_kem1024" },
