...
Deno.test({
  name: "F-STORAGE-05: GUI state.json writes stay private",
  ignore: !isUnix,
  permissions: { read: true, write: true, env: true, net: true },
  fn: async () => {
    await withTestEnv(async (home) => {
      await writeState(`${home}/.ebp`, { currentIdentity: "alice" });
      assertEquals(await statMode(`${home}/.ebp/state.json`), 0o600);
      assertEquals(await statMode(`${home}/.ebp`), 0o700);
    });
  },
});
...
Deno.test({
  ...
  fn: async () => {
    await withTestEnv(async (home, handler) => {
      ...
      assertEquals(
        (body as Record<string, unknown>).protocolVersion,
        PROTOCOL_VERSION,
      );
    });
  },
  ...
  fn: async () => {
    await withTestEnv(async (home, handler) => {
      const { status, body } = await makeRequest(
        handler,
        `/api/v1/context?home=${encodeURIComponent(home)}`,
      );
      assertEquals(status, STATUS.OK);
      const b = body as Record<string, unknown>;
      assertEquals(b.identityDir, undefined);
      assertEquals(b.contactsDir, undefined);
      assertEquals(b.identityDirLabel, "~/.ebp");
      assertEquals(b.contactsDirLabel, "~/.ebp/contacts");
...
Deno.test({
  ...
  fn: async () => {
    await withTestEnv(async (home, handler) => {
      ...
      const { status, body } = await makeRequest(
        handler,
        `/api/v1/identities?home=${encodeURIComponent(home)}`,
      );
      assertEquals(status, STATUS.OK);
      const b = body as {
        identities: Array<{ name: string; fingerprint: string | null }>;
...
Deno.test({
  name:
    "GET /api/v1/identity/public returns public data for new format identity",
  permissions: { read: true, write: true, env: true, net: true },
  fn: async () => {
    await withTestEnv(async (home, handler) => {
      ...
      const { status, body } = await makeRequest(
        handler,
        `/api/v1/identity/public?home=${encodeURIComponent(home)}`,
      );
      assertEquals(status, STATUS.OK);
      const b = body as Record<string, unknown>;
...
Deno.test({
  ...
  fn: async () => {
    await withTestEnv(async (home, handler) => {
      ...
      assertEquals(status, STATUS.OK);
      assertEquals(
        (body as { ok: boolean; currentIdentity: string }).currentIdentity,
        "bob",
      );
    });
  },
...
Deno.test({
  name:
    "POST /api/v1/identity/info returns identity info with correct password",
  permissions: { read: true, write: true, env: true, net: true },
  fn: async () => {
    await withTestEnv(async (home, handler) => {
      const identity = await createTestIdentity(home, "test", "password123");
  ...
  fn: async () => {
    await withTestEnv(async (home, handler) => {
      ...
      const contact: ExternalIdentity = {
        ...
        details: {},
        signingKeyDetails: { variant: "test" },
        encryptionKeyDetails: { variant: "test" },
      };
      await createTestContact(home, "friend", contact);

      const { status, body } = await makeRequest(
        handler,
        `/api/v1/contacts?home=${encodeURIComponent(home)}`,
      );
      assertEquals(status, STATUS.OK);
      const b = body as {
        contacts: Array<{ name: string; fingerprint: string }>;
      };
  ...
  fn: async () => {
    await withTestEnv(async (home, handler) => {
      const validFingerprint =
        (await createTestIdentity(home, "seed", "password123")).toFingerprint();
      const contact: ExternalIdentity = {
        fingerprint: validFingerprint,
        signingKeyType: "dilithium",
        encryptionKeyType: "kyber",
...
Deno.test({
  name:
    "POST /api/v1/sign with includeIdentity embeds public key identity block",
  permissions: { read: true, write: true, env: true, net: true },
  fn: async () => {
    await withTestEnv(async (home, handler) => {
      const identity = await createTestIdentity(home, "test", "password123");
...
Deno.test({
  name:
    "POST /api/v1/verify rejects public identity with invalid signing key type",
  permissions: { read: true, write: true, env: true, net: true },
  fn: async () => {
    await withTestEnv(async (home, handler) => {
      const identity = await createTestIdentity(home, "test", "password123");
...
Deno.test({
  name:
    "POST /api/v1/verify rejects detached signature with wrong message hash",
  permissions: { read: true, write: true, env: true, net: true },
  fn: async () => {
    await withTestEnv(async (home, handler) => {
      const identity = await createTestIdentity(home, "test", "password123");
...
Deno.test({
  name:
    "POST /api/v1/verify returns false for detached signature with mismatched keys",
  permissions: { read: true, write: true, env: true, net: true },
  fn: async () => {
    await withTestEnv(async (home, handler) => {
...
Deno.test({
  ...
  fn: async () => {
    await withTestEnv(async (home, handler) => {
      ...
      const recipient = await createTestIdentity(
        home,
        "recipient",
        "password123",
      );
...
Deno.test({
  name:
    "POST /api/v1/mail/decrypt-attachment decrypts signed attachment payload",
  permissions: { read: true, write: true, env: true, net: true },
  fn: async () => {
    await withTestEnv(async (home, handler) => {
      const sender = await createTestIdentity(home, "sender", "password123");
      ...
      const ciphertext = sender.signAndEncryptFor(
...
Deno.test({
  ...
  fn: async () => {
    await withTestEnv(async (home, handler) => {
      ...
      const { status, body } = await makeRequest(
        handler,
        `/api/v1/server?home=${encodeURIComponent(home)}`,
      );
      assertEquals(status, STATUS.OK);
...
Deno.test({
  ...
  fn: async () => {
    await withTestEnv(async (home, handler) => {
      ...
      assertEquals(status, STATUS.OK);
      assertEquals(
        (body as { ok: boolean; server: string }).server,
        "http://new-server.com",
      );
    });
  },
...
Deno.test({
  ...
  fn: async () => {
    await withTestEnv(async (home, handler) => {
      ...
      assertEquals(status, STATUS.OK);
      assertEquals(
        (body as { ok: boolean; server: string | null }).server,
        null,
      );
    });
  },
...
Deno.test({
  name:
    "GET /api/v1/server/identities with query param passes through to search endpoint",
  permissions: { read: true, write: true, env: true, net: true },
  fn: async () => {
    await withTestEnv(async (home, handler) => {
