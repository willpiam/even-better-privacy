...
export const MAIL_OAUTH_MAX_PENDING_STARTS = 64;
export const MAIL_OAUTH_START_RATE_LIMIT = 20;
const MAIL_OAUTH_START_RATE_WINDOW_MS = 60 * 1000;
...
const mailOauthStartWindows = new Map<
  string,
  { windowStartedAt: number; count: number }
>();

let _oauthProviderConfigs:
  | Record<Exclude<MailOauthProvider, "">, OAuthProviderConfig>
  | null = null;

function ensureProviderConfigs(): Record<
...
export function getOAuthProviderConfigs(): Record<
  Exclude<MailOauthProvider, "">,
  OAuthProviderConfig
> {
...
export function isAllowedMailOAuthBrowserUrl(value: string): boolean {
  let target: URL;
  try {
  ...
  const allowedHosts = new Set(
    Object.values(ensureProviderConfigs()).map((conf) =>
      new URL(conf.authUrl).hostname.toLowerCase()
    ),
  );
...
export function registerMailOAuthStart(
  oauthState: string,
  pending: PendingMailOAuthStart,
  clientKey: string,
  now = Date.now(),
): void {
  pruneExpiredOAuthState();

  const startWindow = mailOauthStartWindows.get(clientKey);
  if (
    startWindow &&
    now - startWindow.windowStartedAt < MAIL_OAUTH_START_RATE_WINDOW_MS
  ) {
    if (startWindow.count >= MAIL_OAUTH_START_RATE_LIMIT) {
      throw new HttpError(
        STATUS.TooManyRequests,
        "too many mail oauth starts; try again shortly",
      );
    }
    startWindow.count += 1;
  } else {
    mailOauthStartWindows.set(clientKey, { windowStartedAt: now, count: 1 });
  }

  if (mailOauthStarts.size >= MAIL_OAUTH_MAX_PENDING_STARTS) {
    throw new HttpError(
      STATUS.TooManyRequests,
  ...
  mailOauthStarts.set(oauthState, pending);
}

export function getMailOAuthRedirectUri(): string {
  return `http://127.0.0.1:${
    Number(Deno.env.get("GUI_BACKEND_PORT") ?? "8787")
...
> {
  ...
  const refreshToken = typeof body.refreshToken === "string"
    ? body.refreshToken
    : "";
  const tokenExpiry = typeof body.tokenExpiry === "number"
    ? body.tokenExpiry
    : Date.now() + 3600_000;
  ...
  if (!accessToken || !refreshToken || !email) {
    throw new HttpError(
      STATUS.BadGateway,
      "oauth token response missing required fields",
    );
  }
...
export async function refreshOAuthToken(
  ...
  if (!secrets.refreshToken) {
    throw new HttpError(
      STATUS.BadRequest,
      "mail oauth refresh token is not configured",
    );
  }
  ...
  if (!serverUrl) {
    throw new HttpError(
      STATUS.BadRequest,
      "server is not configured; needed for oauth token refresh",
    );
  }
  ...
  if (!res.ok) {
    ...
    throw new HttpError(
      STATUS.BadGateway,
      `oauth token refresh failed: ${reason}`,
    );
  }
  const accessToken = typeof body.accessToken === "string"
    ? body.accessToken
  ...
  const updated: MailAuthSecrets = {
    ...secrets,
    accessToken,
    tokenExpiry: typeof body.tokenExpiry === "number"
      ? body.tokenExpiry
      : Date.now() + 3600_000,
