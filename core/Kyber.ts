import { AsymmetricEncryptionKey } from "./Keys.ts";
...
export class KyberEncryptionKey extends AsymmetricEncryptionKey {
  ...
  static EncryptFor(recipient: ExternalIdentity, message: string): string {
    ...
    const variant =
      ((kyber as unknown) as Record<string, KyberVariant>)[variantName];
    ...
    const { cipherText: encapsulatedKey, sharedSecret } = variant.encapsulate(
      recipientPublicKey,
    );
  ...
  static fromPublicKey(
    publicKey: string,
    variant: string = "ml_kem1024",
  ): KyberEncryptionKey {
    const instance = Object.create(
      KyberEncryptionKey.prototype,
    ) as KyberEncryptionKey;
