...
import { hexToString, stringToHex } from "./Hex.ts";
...
export const MAX_REVOCATION_REASON_LENGTH = 1024;
...
export function createRevocationCertificate(
  ...
  if (
    ...
    options.reason.length > MAX_REVOCATION_REASON_LENGTH
  ) {
    throw new Error(
      `reason exceeds maximum length (${MAX_REVOCATION_REASON_LENGTH})`,
    );
  }
  return {
    type,
 ...
 */
export function getRevocationSignaturePayload(
  cert: RevocationCertificateData,
): string {
 ...
 */
export function encodeRevocationCertificate(
 ...
 */
export function decodeRevocationCertificate(
  encoded: string,
  ...
  try {
    ...
    if (
      ...
      (cert.reason !== undefined && typeof cert.reason !== "string") ||
      (typeof cert.reason === "string" &&
        cert.reason.length > MAX_REVOCATION_REASON_LENGTH) ||
...
export function verifyRevocationCertificate(
  ...
  if (
    typeof cert.nonce !== "number" || !Number.isInteger(cert.nonce) ||
    cert.nonce < 0
  ) {
  ...
  if (cert.reason !== undefined && typeof cert.reason !== "string") {
  ...
  if (
    ...
    return {
      ok: false,
      error: `reason exceeds maximum length (${MAX_REVOCATION_REASON_LENGTH})`,
    };
  }
  ...
  const legacyEnvelope = buildLegacyMessageHashEnvelopeFromHash(
    sha256Hex(payload),
  );
  let verified = false;
...
export function decodeAndVerifyRevocationCertificate(
  ...
  if (input.expectedType && cert.type !== input.expectedType) {
    return {
      ok: false,
      error: `expected ${input.expectedType} revocation, got ${cert.type}`,
    };
  }
  if (
    input.expectedFingerprint && cert.fingerprint !== input.expectedFingerprint
  ) {
    return { ok: false, error: "certificate fingerprint mismatch" };
  }
  if (
    input.expectedType === "detail" && input.expectedTarget &&
    cert.target !== input.expectedTarget
  ) {
 ...
 */
export function isValidRevocationNonce(
  nonce: number,
  maxSeenNonce: number,
): boolean {
