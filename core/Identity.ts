import { AsymmetricEncryptionKey, SigningKey } from "./Keys.ts";
...
function parseIdentityStorageJson(storageData: string): {
  storage: IdentityStorageFormat;
  pub: IdentityPublicData;
} {
  let parsed: unknown;
  try {
  ...
  if (pub.signingKeyType !== "dilithium" && pub.signingKeyType !== "sphincs") {
    throw new StorageFormatError(
  ...
  return { storage: storage as IdentityStorageFormat, pub };
}

function buildIdentityShell(pub: IdentityPublicData): Identity {
  ...
  identity.revokedDetails = new Map(
    Object.entries(pub.revokedDetails ?? {}) as [string, string][],
  );
...
function applyPrivateKeysFromJson(
  identity: Identity,
  pub: IdentityPublicData,
  privateJson: string,
): void {
...
function applyPublicOnlyKeys(identity: Identity, pub: IdentityPublicData): void {
...
export class Identity extends Key {
  ...
  static EncryptFor(recipient: ExternalIdentity, message: string): string {
    switch (recipient.encryptionKeyType) {
      ...
      default:
        throw new Error(
          `Unsupported encryption key type: ${recipient.encryptionKeyType}`,
        );
    }
  }
  ...
  static VerifySignature(
    ...
    if (purpose === "message") {
      const legacy = buildLegacyMessageHashEnvelopeFromHash(
        sha256Hex(message),
        salt,
      );
  ...
  static VerifyRecipientBoundSignature(
    ...
    const envelope = buildRecipientBoundEnvelope(
  ...
  private static _VerifyEnvelope(
    sender: ExternalIdentity,
    envelope: string,
    signature: string,
  ): boolean {
    switch (sender.signingKeyType) {
      case "dilithium":
        return DilithiumSigningKey.verify(
          sender.signingKeyDetails.variant,
          envelope,
          signature,
          sender.signingKey,
        );
      case "sphincs":
        return SphincsSigningKey.verify(
          sender.signingKeyDetails.variant,
          envelope,
          signature,
          sender.signingKey,
      ...
      default:
        throw new Error(
          `Unsupported signing key type: ${sender.signingKeyType}`,
        );
    }
  }
  ...
  constructor(
    ...
    switch (encryptionKeyType) {
      ...
      default:
        throw new Error(
          `Unsupported encryption key type: ${encryptionKeyType}`,
        );
    }
  ...
  signAndEncryptMessage(message: string, recipient: Identity): string {
    ...
    const signature = this.signMessageForRecipient(
      message,
      recipientFingerprint,
    );
    return recipient.encryptionKey.encrypt(JSON.stringify({
      type: "ebp-encrypted-signed-inner",
      message,
      signature,
  ...
  signAndEncryptForMany(
    ...
    const signature = this.signMessageForRecipients(
      message,
      recipientFingerprints,
      attachmentManifest,
    );
    const innerPayload = JSON.stringify({
      type: "ebp-encrypted-signed-inner-multi",
      message,
      signature,
      envelopeVersion: 3,
      recipientFingerprints,
    ...
    const contentKey = options?.contentKeyHex
      ? hexToBytes(options.contentKeyHex)
      : undefined;
  ...
  decryptAndVerify(
    ...
    message: string;
    verified: boolean;
    verifyStatus: "valid" | "valid_unbound" | "invalid";
  } {
    ...
    if (parsed.envelopeVersion === 2) {
      if (
        ...
        parsed.type !== "ebp-encrypted-signed-inner"
      ) {
      ...
      if (
        typeof parsed.recipientFingerprint === "string" &&
  ...
  decryptAndVerifyMulti(
    ...
    const myFingerprint = this.toFingerprint();
    const recipientEntry = payload.recipients.find((entry) =>
      entry.fingerprint === myFingerprint
    );
    if (!recipientEntry) {
      return {
        message: "",
    ...
    const recipientFingerprints = Array.isArray(parsed.recipientFingerprints)
      ? parsed.recipientFingerprints.filter((value): value is string =>
        typeof value === "string"
      ).sort()
      : [];
    const attachmentManifest = Array.isArray(parsed.attachmentManifest)
      ...
      : [];
    if (
      parsed.envelopeVersion !== 3 ||
      (parsed.type !== undefined &&
        parsed.type !== "ebp-encrypted-signed-inner-multi") ||
      !recipientFingerprints.includes(myFingerprint)
    ) {
      return {
        message,
        verified: false,
  ...
  attachDetail(path: string, detail: string) {
    ...
    const detailRecordSignature = this.signMessage(
      canonicalJsonStringify(detailRecord),
      undefined,
      "detail-proof",
    );
  ...
  getDetail(path: string): string | null {
    ...
    try {
      ...
      const isValid = this.verifyMessage(
        signedPayload,
        signature,
        undefined,
        "detail-proof",
  ...
  static VerifyDetails(details: Map<string, [string, string]>): boolean {
    ...
    for (const [path, [detail, proof]] of details.entries()) {
      try {
        ...
        if (
          typeof record.path !== "string" || typeof record.detail !== "string"
        ) {
          return false;
        }
  ...
  revokeDetail(path: string, reason?: string): string {
    ...
    const cert = createRevocationCertificate(
      "detail",
      this.toFingerprint(),
   ...
   */
  getDetailRevocationCertificate(
    path: string,
  ): SignedRevocationCertificate | null {
  ...
  static fromJSON(json: string): Identity {
    ...
    switch (data.encryptionKeyType) {
      ...
      default:
        throw new Error(
          `Unsupported encryption key type: ${i.encryptionKeyType}`,
        );
    }

    const detailsEntries = Object.entries(data.details ?? {}) as [
      string,
      [string, string],
    ][];
    i.details = new Map(detailsEntries);

    const storedNonce = typeof data.detailsNonce === "number" &&
        Number.isFinite(data.detailsNonce)
      ? data.detailsNonce
      : undefined;
    ...
    const revokedDetailsEntries = Object.entries(data.revokedDetails ?? {}) as [
      string,
      string,
    ][];
  ...
  private privateStorageJson(): string {
  ...
  /** Store with a pre-derived AES key (mobile native Argon2 path). */
  toStorageFormatWithKey(key: Uint8Array, salt: Uint8Array): string {
    const encrypted = AES.encryptWithKey(
      key,
      this.privateStorageJson(),
      salt,
      identityStorageAad(),
  ...
  static isStorageEncryptedWithLegacyKDF(storageData: string): boolean {
    try {
      ...
      if (
        parsed.version === FILE_FORMAT_VERSIONS.identityStorage &&
  ...
  static readPublicData(storageData: string): IdentityPublicData | null {
    try {
      const parsed = JSON.parse(storageData);
      if (
        parsed.version === FILE_FORMAT_VERSIONS.identityStorage && parsed.public
      ) {
   ...
   */
  /**
   * Load identity with a pre-derived AES key (mobile native Argon2 path).
   */
  static fromStorageFormatWithKey(
    storageData: string,
    key: Uint8Array,
  ): PrivateIdentity {
    const { storage, pub } = parseIdentityStorageJson(storageData);
    if (typeof storage.encrypted !== "string") {
      throw new StorageFormatError("Missing encrypted private identity data");
    }
    const identity = buildIdentityShell(pub);
    let privateJson: string;
    try {
      privateJson = AES.decryptWithKey(
        key,
        storage.encrypted,
        identityStorageAad(),
      );
    ...
    applyPrivateKeysFromJson(identity, pub, privateJson);
    return identity as PrivateIdentity;
  }
  ...
  static fromStorageFormat(
    ...
    if (password) {
      ...
      applyPublicOnlyKeys(identity, pub);
    }
