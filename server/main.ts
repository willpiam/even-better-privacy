...
function parseVerifyInput(payload: Record<string, unknown>): { ok: true; data: VerifyInput } | { ok: false; error: string } {
  ...
  if (chosenPublicIdentity !== null && chosenPublicIdentity !== undefined) {
    ...
    if (signingKeyType !== "dilithium" && signingKeyType !== "sphincs") {
      return { ok: false, error: "publicIdentity has invalid signing key type" };
    }
    ...
    const signingVariant = (signingKeyDetails as Record<string, unknown>).variant;
  ...
  const resolvedFingerprint = publicIdentity?.fingerprint ?? (fingerprintCheck.value || null);
  return {
    ok: true,
    data: {
...
async function handleVerifySignature(req: Request, db: DatabaseAdapter): Promise<Response> {
  const bodyResult = await readJsonBody<Record<string, unknown>>(req);
  if (!bodyResult.ok) {
    return json({ error: bodyResult.error }, bodyResult.status);
  }
  ...
  if (publicIdentity) {
    ...
    if (!fingerprint) {
      return json({ error: "fingerprint or publicIdentity is required for verification" }, 400);
    }
    const identity = await getIdentity(db, fingerprint);
    if (!identity) {
      return json({ error: "identity not found" }, 404);
...
async function handlePostHierarchyPropose(req: Request, db: DatabaseAdapter): Promise<Response> {
...
async function handleGetHierarchyPending(fingerprint: string, db: DatabaseAdapter): Promise<Response> {
  if (fingerprint.length > LIMITS.fingerprint) {
    return json({ error: "fingerprint too long" }, 400);
  }
  if (!isValidFingerprintBech32(fingerprint)) {
...
async function handlePostHierarchyAccept(req: Request, db: DatabaseAdapter): Promise<Response> {
...
async function handlePostHierarchyReject(req: Request, db: DatabaseAdapter): Promise<Response> {
