    ...
    tauri::Builder::default()
        ...
        .setup({
            ...
            move |app| {
                ...
                append_log_line(
                    log_file.as_mut(),
                    &format!("resolved sidecar: {}", sidecar_path.display()),
                );

                let mut cmd = Command::new(&sidecar_path);
                cmd.env("GUI_BACKEND_HOST", "127.0.0.1")
                    .env("GUI_BACKEND_PORT", "8787");
                set_env_if_non_empty(
                    &mut cmd,
                    "MAIL_OAUTH_GMAIL_CLIENT_ID",
                    env::var("MAIL_OAUTH_GMAIL_CLIENT_ID").ok().as_deref(),
                );
                set_env_if_non_empty(
                    &mut cmd,
                    "MAIL_OAUTH_OUTLOOK_CLIENT_ID",
                    env::var("MAIL_OAUTH_OUTLOOK_CLIENT_ID").ok().as_deref(),
                );
...
fn resolve_sidecar(app: &tauri::App) -> Result<PathBuf, tauri::Error> {
    ...
    let mut candidates = vec!["bin/ebp-gui-backend", "ebp-gui-backend"];
    if let Some(target_sidecar) = target_sidecar {
        candidates.insert(0, target_sidecar);
        if let Some(file_name) = Path::new(target_sidecar)
            .file_name()
            .and_then(|s| s.to_str())
        {
    ...
    if let Ok(appdir) = env::var("APPDIR") {
        ...
        for candidate in &expanded_candidates {
            let path = appdir_path
                .join("usr/bin")
                .join(Path::new(candidate).file_name().unwrap());
            if let Some(found) = try_path(path) {
                return Ok(found);
            }
        }
    ...
    Err(tauri::Error::AssetNotFound(format!(
        "sidecar binary not found in bundled resources; refusing executable-directory fallback. attempted: {}",
        attempted_paths
            .iter()
