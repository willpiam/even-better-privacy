...
export async function cmdPublishIdentity(args: ReturnType<typeof parseArgs>, ctx: CLIContext): Promise<void> {
...
export async function cmdFetchIdentity(args: ReturnType<typeof parseArgs>, ctx: CLIContext): Promise<void> {
	const server = ensureServer(ctx, args);
	...
	try {
		...
		const preservedEntries = Object.entries(existing.resolvedOpaqueDetails ?? {}).filter(
			([path, value]) => typeof value === "string" && external.details[path] !== undefined,
		);
...
export async function cmdListServerIdentities(args: ReturnType<typeof parseArgs>, ctx: CLIContext): Promise<void> {
	const server = ensureServer(ctx, args);
...
export async function cmdServer(args: ReturnType<typeof parseArgs>, ctx: CLIContext): Promise<void> {
