...
export async function cmdGenerate(
  ...
  if (!["dilithium", "sphincs"].includes(signingType)) {
    console.error(
      `Invalid signing key type: ${signingType}. Use 'dilithium' or 'sphincs'.`,
    );
    Deno.exit(1);
  }
  if (encryptionType !== "kyber") {
    console.error(
      `Invalid encryption key type: ${encryptionType}. Only 'kyber' is supported.`,
    );
    Deno.exit(1);
  }

  console.log(
    `Generating identity with ${signingType} signing and ${encryptionType} encryption...`,
  );
  ...
  const password = await readPassword(
    "Enter password to protect your identity: ",
  );
  ...
  if (!passwordCheck.ok) {
    ...
    for (const suggestion of passwordCheck.suggestions) {
...
export async function cmdExportPublic(
  args: ReturnType<typeof parseArgs>,
  ctx: CLIContext,
): Promise<void> {
  const { identity } = await loadIdentity(
...
export async function cmdUseIdentity(
  args: ReturnType<typeof parseArgs>,
  ctx: CLIContext,
): Promise<void> {
...
export async function cmdShowDetails(
  args: ReturnType<typeof parseArgs>,
  ctx: CLIContext,
): Promise<void> {
  const { identity } = await loadIdentity(
