...
export async function updateState(
  identityDir: string,
  updates: Partial<CLIState>,
): Promise<CLIState> {
  const existing = (await readState(identityDir)) ??
...
export async function listIdentityNames(
  identityDir: string,
): Promise<string[]> {
  try {
...
export async function getContext(
  ...
  const currentIdentity = identityOverride ?? state?.currentIdentity ??
    "identity";
...
export async function fixLegacyPerms(identityDir: string): Promise<void> {
  ...
  try {
    for await (const entry of Deno.readDir(identityDir)) {
      ...
      if (entry.isFile) {
        try {
          await Deno.chmod(childPath, 0o600);
        } catch { /* best-effort */ }
...
export function buildStateFromExternal(
...
export function ensureServer(ctx: CLIContext, args: ParsedArgs): string {
  ...
  if (!server) {
    console.error(
      "No server configured. Set one with: ebp server <url> (or pass --server <url>)",
    );
    Deno.exit(1);
  }
  try {
...
export async function loadIdentity(
  ctx: CLIContext,
  password?: string,
): Promise<{ identity: Identity; password: string }> {
